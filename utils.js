...
//make a move
function move(ele){
    var move_type = check_move(ele.innerHTML);
    if(move_type!=null){
        document.getElementById("score_no").innerHTML=++score;
        switch(move_type){
            case "left":
                cell_state[ele.innerHTML]["left"]+=102;
                ele.style.left=cell_state[ele.innerHTML]["left"];
            break;
            case "right":
                cell_state[ele.innerHTML]["left"]-=102;
                ele.style.left=cell_state[ele.innerHTML]["left"];
            break;
            case "top":
                cell_state[ele.innerHTML]["top"]+=102;
                ele.style.top=cell_state[ele.innerHTML]["top"];
            break;
            case "bottom":
                cell_state[ele.innerHTML]["top"]-=102;
                ele.style.top=cell_state[ele.innerHTML]["top"];
